...
public class QuestionFactoryTest 
{
	...
	//	this.questionFactory = new QuestionFactory();
		...
		questionMethodCallConstructor = new Microtask(CodeElement.METHOD_INVOCATION, 
				this.codeSnippetConstructor, "Is there maybe something wrong in the declaration of function "
...
//		this.questionFactory.addConcreteQuestion(constructorBody, this.codeSnippetConstructor);
		...
		questionIf11 = new Microtask(CodeElement.IF_CONDITIONAL, this.codeSnippetConstructor, 
				"Is it possible that the conditional clause at line 8 has problems (e.g., wrong Boolean operator, "
						+ "wrong comparison, misplaced parentheses, etc.)?", 8, 0, 8, 11,3);
//		this.questionFactory.addConcreteQuestion(questionIf11, this.codeSnippetConstructor);
		...
		questionIf12 = new Microtask(CodeElement.IF_CONDITIONAL, this.codeSnippetConstructor, 
				"Is there maybe something wrong with the body of the conditional clause at line 8 (e.g., enters the "
						+ "wrong branch, makes a call to a null pointer, calls a wrong type, etc.)?", 9, 0, 9, 15,4);
//		this.questionFactory.addConcreteQuestion(questionIf12, this.codeSnippetConstructor);
		...
		questionMethodCallFactorial = new Microtask(CodeElement.METHOD_INVOCATION, 
				this.codeSnippetFactorial, "Is there maybe something wrong in the declaration of function 'factorial' "
						+ "at line 20 (e.g., requires a parameter that is not listed, needs different parameters to "
						+ "produce the correct result, specifies the wrong or no return type, etc .)?", 20, 0, 20, 58,5);
//		this.questionFactory.addConcreteQuestion(questionMethodCallFactorial, this.codeSnippetFactorial);
		...
		factorialBody = new Microtask(CodeElement.METHOD_DECLARATION, this.codeSnippetFactorial, "Is there possibly "
				...
				+ "handle error situations, etc.)?", 20, 58, 30, 1,6);
//		this.questionFactory.addConcreteQuestion(questionMethodCallFactorial, this.codeSnippetFactorial);

		questionIf21 = new Microtask(CodeElement.IF_CONDITIONAL, this.codeSnippetFactorial, 
				"Is it possible that the conditional clause at line 21 has problems (e.g., wrong Boolean operator, "
						+ "wrong comparison, misplaced parentheses, etc.)?", 21, 0, 21, 15,7);
//		this.questionFactory.addConcreteQuestion(questionIf21, this.codeSnippetFactorial);
		...
		questionIf22 = new Microtask(CodeElement.IF_CONDITIONAL, this.codeSnippetFactorial, 
				"Is there maybe something wrong with the body of the conditional clause between lines 22 and 29 "
				+ "(e.g., enters the wrong branch, makes a call to a null pointer, "
				+ "calls a wrong type, etc.)?", 22, 3, 22, 17,8);
	//	this.questionFactory.addConcreteQuestion(questionIf22, this.codeSnippetFactorial);
		...
		questionFor1 = new Microtask(CodeElement.FOR_LOOP, this.codeSnippetFactorial, 
				"Is there maybe something wrong with the 'For-loop' construct at line 23 (e.g., incorrect "
						+ "initialization, wrong counter increment, wrong exit condition, etc.)?", 23, 0, 23, 42,9);
	//	this.questionFactory.addConcreteQuestion(questionFor1, this.codeSnippetFactorial);
		...
		questionFor2 = new Microtask(CodeElement.FOR_LOOP, this.codeSnippetFactorial, 
				"Is the body of the 'For-loop' between lines 23 and 25 possibly not producing what it is supposed to "
	...
	//	this.questionFactory.addConcreteQuestion(questionFor2, this.codeSnippetFactorial);
		...
		questionInvocation1 = new Microtask(CodeElement.METHOD_INVOCATION, this.codeSnippetFactorial, "Is there perhaps something wrong with the "
				...
				+ "values of the parameters are not checked, etc .)?", 23, 26, 23, 36,11);
	//	this.questionFactory.addConcreteQuestion(questionFor2, this.codeSnippetFactorial);	
	}
	...
	public void test() {
		...
		if((list ==null ) || (list.size()!=2))
			...
			if((snippet1.isEqualTo(codeSnippetConstructor)) || (snippet1.isEqualTo(codeSnippetFactorial)) && 
				...
				this.questionFactory.generateQuestions(list,"Failure message",0);
